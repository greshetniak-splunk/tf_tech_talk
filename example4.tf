...
resource "scp_indexes" "my_scp_index" {   //this is a Terrafrom variable
...
resource "scp_users" "github_app_admin_user" {
  ...
  default_app                 = scp_splunkbase_app.name
  roles                       = ["user"]
  federated_search_manage_ack = "Y"
  email                       = "tester@domain.com"
}

// Terraform automatically infers a dependency — a user won't be created until application exists.
